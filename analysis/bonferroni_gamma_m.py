...
Standard library only. Reads `ks_null_summary.csv` (the five Table-18 substrates with
...
Two thresholds (see STATISTICS.md):
  * strict   p < 1e-3   -> the headline "all five main substrates"
...
def ks_pvalue(d, n1, n2):
    ...
    ne = n1 n2 / (n1 + n2).  (Exact for large n; over-estimates p for very small n.)"""
    ne = n1 * n2 / (n1 + n2)
...
def main():
    ...
    print(f"unter gamma_M = {GAMMA_M:.5f}:          {n_gamma}/{n}   "
          f"(alle fuenf unter {GAMMA_M:.5f})")
    print()
    ok = (n_strict == 5 and n_gamma == 5)
    print("REPRODUKTION DER HEADLINE:", "OK (5/5 strikt, 5/5 unter gamma_M)" if ok else "ABWEICHUNG")
    print("Hinweis: die historische '19 von 26'-Zaehlung war der gamma_M-Count ueber die")
    print("erweiterte 26-Substruktur-Aufstellung (S8), NICHT ueber diese fuenf Haupt-Substrate.")
